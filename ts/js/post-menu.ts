import { POST_BODY_INPUT, POST_MENU_CLOSE_BUTTON, POST_MENU_SAVE_BUTTON, POST_TAGS_INPUT, POST_TITLE_INPUT } from "./components/post-menu.ts";
...
export function setSavePostButtonEvent(): void {
    const savePostButton: HTMLElement = getHTMLElement("#" + POST_MENU_SAVE_BUTTON);
...
export function setClosePostMenuButtonEvent(): void {
    const closePostMenuButton: HTMLElement = getHTMLElement("#" + POST_MENU_CLOSE_BUTTON);
...
export function getPostMenuInputs(): PostMenuInput {
    return {
        ...
        tags: (getHTMLElement("#" + POST_TAGS_INPUT) as HTMLInputElement).value.split(",")
    }
}